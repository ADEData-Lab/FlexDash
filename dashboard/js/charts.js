...
function initializeCharts() {
    ...
    createAssetGroupChart(data);
    createAssetGroupCountChart(data);
    ...
    // External benchmarks (CM sense-check)
    createBenchmarkDeltaChart(data);
    createCMBenchmarkChart(data);
}
/**
...
/**
 * Create asset group horizontal bar chart (broader categories)
 */
function createAssetGroupChart(data) {
    const ctx = document.getElementById('asset-group-chart');
    if (!ctx) return;

    const groupData = data.asset_group_breakdown || [];
    if (!groupData || groupData.length === 0) {
        return;
    }

    // Sort by capacity descending (use midpoint for illustrative ranges)
    const sorted = [...groupData].sort((a, b) => {
        const aVal = a.capacity_mw_illustrative && a.capacity_range_max ?
            (a.capacity_range_min + a.capacity_range_max) / 2 : (a.capacity_mw ?? 0);
        const bVal = b.capacity_mw_illustrative && b.capacity_range_max ?
            (b.capacity_range_min + b.capacity_range_max) / 2 : (b.capacity_mw ?? 0);
        return bVal - aVal;
    });

    const labels = sorted.map(d => {
        let label = d.display_name || formatLabelLocal(d.asset_group);
    ...
    const values = sorted.map(d => {
        if (d.capacity_mw_illustrative && d.capacity_range_max != null) {
            return (d.capacity_range_min + d.capacity_range_max) / 2;
        }
        return d.capacity_mw ?? 0;
    });
                ...
                tooltip: {
                    ...
                    callbacks: {
                        title: function(context) {
                            ...
                            return sorted[idx].display_name || formatLabelLocal(sorted[idx].asset_group);
                        },
                        label: function(context) {
                            ...
                            const v = sorted[idx].capacity_mw ?? 0;
                            return `${new Intl.NumberFormat('en-GB').format(v)} MW (k=${k})`;
                        }
                    }
                }
            },
                ...
                x: {
                    ...
                    ticks: {
                        callback: function(value) {
                            return value >= 1000 ? (value / 1000) + ' GW' : value + ' MW';
                        }
                    }
                },
                y: {
    ...
    const noteEl = document.getElementById('asset-group-note');
    if (noteEl && illustrativeFlags.some(Boolean)) {
        noteEl.textContent = 'Some values are shown as ranges due to disclosure control.';
    }
...
/**
 * Create asset group counts chart (portfolio sizes)
 */
function createAssetGroupCountChart(data) {
    const ctx = document.getElementById('asset-group-count-chart');
    if (!ctx) return;
    ...
    const sorted = [...groupData].sort((a, b) => {
        const aVal = a.count_illustrative && a.count_range_max ?
            (a.count_range_min + a.count_range_max) / 2 : (a.count ?? 0);
        const bVal = b.count_illustrative && b.count_range_max ?
            (b.count_range_min + b.count_range_max) / 2 : (b.count ?? 0);
        return bVal - aVal;
    });

    const labels = sorted.map(d => {
        let label = d.display_name || formatLabelLocal(d.asset_group);
        if (d.count_illustrative && d.count_range) {
            label += ` [${d.count_range}]`;
        }
        return label;
    });
    ...
    const values = sorted.map(d => {
        let val;
        if (d.count_illustrative && d.count_range_max != null) {
            val = (d.count_range_min + d.count_range_max) / 2;
        } else {
            val = d.count ?? 0;
        }
        return Math.max(Number(val) || 0, 1); // keep log-scale stable
    });

    const illustrativeFlags = sorted.map(d => d.count_illustrative);
    const ranges = sorted.map(d => d.count_range);
    const kValues = sorted.map(d => d.k_count ?? d.k ?? 0);

    const colors = sorted.map((d, i) =>
        d.count_illustrative ? CHART_COLORS.illustrative : CHART_COLORS.palette[i % CHART_COLORS.palette.length]
    );
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        label: function(context) {
                            ...
                            const v = sorted[idx].count ?? 0;
                            return `${new Intl.NumberFormat('en-GB').format(v)} (k=${k})`;
                        }
                    }
                }
            },
    ...
    const noteEl = document.getElementById('asset-group-count-note');
    if (noteEl && illustrativeFlags.some(Boolean)) {
        noteEl.textContent = 'Some counts are shown as ranges due to disclosure control.';
    }
...
/**
 * Benchmarks: Delta chart (contributors - CM) by category
 */
function createBenchmarkDeltaChart(data) {
    const ctx = document.getElementById('benchmark-delta-chart');
    if (!ctx) return;

    const rows = data?.benchmarks?.comparison || [];
    if (!rows.length) return;

    const labels = rows.map(r => String(r.asset_class || ''));
    const valuesMW = rows.map(r => (Number(r.variance_kw) || 0) / 1000);

    const styles = getComputedStyle(document.documentElement);
    const posColor = styles.getPropertyValue('--chart-illustrative').trim() || CHART_COLORS.illustrative;
    const negColor = styles.getPropertyValue('--chart-ic').trim() || CHART_COLORS.secondary;
    const bg = valuesMW.map(v => (v >= 0 ? posColor : negColor));
    ...
    if (!window.DashboardState.charts.benchmarks) {
        window.DashboardState.charts.benchmarks = [];
    }
    window.DashboardState.charts.benchmarks.push(chart);
}

/**
 * Benchmarks: CM benchmark totals by category
 */
function createCMBenchmarkChart(data) {
    const ctx = document.getElementById('cm-benchmark-chart');
    if (!ctx) return;

    const rows = data?.benchmarks?.cm_benchmark_by_asset || [];
    if (!rows.length) return;

    const labels = rows.map(r => String(r.asset_class || ''));
    const valuesMW = rows.map(r => (Number(r.cm_benchmark_kw) || 0) / 1000);

    const styles = getComputedStyle(document.documentElement);
    const primary = styles.getPropertyValue('--chart-domestic').trim() || CHART_COLORS.primary;
...
function createDomesticAssetChart(data) {
    ...
    // Prefer sector x asset-group breakdown (more disclosure-safe), then fall back to asset-class.
    const useGroups = Array.isArray(data.sector_asset_group_breakdown) && data.sector_asset_group_breakdown.length > 0;
    const assetData = useGroups
        ? data.sector_asset_group_breakdown
        : (data.sector_asset_breakdown?.length ? data.sector_asset_breakdown : (data.asset_breakdown || []));
    const key = useGroups ? 'asset_group' : 'asset_class';

    const filtered = assetData.filter(d => d.sector === 'domestic');
    ...
    const labels = sorted.map(d => {
        let label = d.display_name || formatLabelLocal(d[key]);
        if (d.capacity_mw_illustrative && d.capacity_range) {
            label += ` [${d.capacity_range}]`;
        }
        return label;
...
function createICAssetChart(data) {
    ...
    const filtered = assetData.filter(d => d.sector === 'ic');
