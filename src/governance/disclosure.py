...
class DisclosureController:
    ...
    def apply_disclosure_control(
        ...
        for group_key, group_df in df.groupby(dimension_cols):
            contributors = group_df[contributor_col].unique().tolist()
            k = len(contributors)
            ...
            row_data['_contributor_count'] = k
            is_safe = True

            for metric in metric_cols:
                agg_value = group_df[metric].sum()
                contributor_values = group_df.groupby(contributor_col)[metric].sum().tolist()
